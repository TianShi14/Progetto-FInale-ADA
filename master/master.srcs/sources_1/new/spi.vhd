...
architecture behavioral of spi is
    ...
    signal toggles       : integer range 0 to width * 2 + 1;        -- identificatore delle modalit� e transizioni del clock SPI
...
begin
    ...
    begin
        ...
        if reset = '0' then                                         -- reset     
            busy    <= '1';   
            cs      <= '1';                                         -- Chip Select alto quando la periferica � inusata (SPI reference)
            mosi    <= 'Z';                                         
            receive <= (others => '0'); 
            state   <= start;          
             ...
             case state is               
                when start =>
                    ...
                    if enable = '1' then                            -- l'inizio � stato segnalato
                        busy <= '1';          
                        if divider = 0 then                         -- se 0 abbiamo una configurazione non valida per sclk, viene settato alla massima velocit�
                            ratio   <= 1;                           
                            counter <= 1;     
                        else
                ...
                when execute =>
                    ...
                    if counter = ratio then                                                            -- il ratio tra clock di sistema e sclk � stato raggiunto
                        ...
                        if toggles = last_data_bit and cont = '1' then                                  -- � stata segnalata la modalit� continua, si ricarica il buffer in uscita e si aggiunge un flag continue per il prossimo half cycle di sclk
                            out_buffer <= transmit;                       
                    ...
                    else                                                                                -- se non � raggiunto il ratio si incrementa il contatore
                        counter <= counter + 1;
                        state   <= execute;
                    end if;
             end case;
