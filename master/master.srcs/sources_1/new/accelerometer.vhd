...
entity accelerometer is
    ...
    port(
        ...
        cordic_ena     : out     std_logic;                      -- abilita il calcolo dell'atan2
        acceleration_x : out     std_logic_vector(15 downto 0);  -- accelerazione asse x
        acceleration_y : out     std_logic_vector(15 downto 0)   -- accelerazione asse y
    );    
end accelerometer;
...
architecture behavioral of accelerometer is
    ...
    signal spi_cont           : std_logic;                              -- flag per modalit� continua (SPI reference)
    ...
    component spi is
        ...
        port(
            clk      : in     std_logic;                                -- clock di sistema                       
            reset    : in     std_logic;                                -- reset asincrono attivo basso
            cpol     : in     std_logic;                                -- polarit� (SPI reference)
            ...
            transmit : in     std_logic_vector(width - 1 downto 0);     -- dati da trasmettere (indirizzi registri, sensibilit� acquisizione dati, etc.)                     
            sclk     : buffer std_logic;                                -- clock SPI (SPI reference)
            cs       : buffer std_logic;                                -- chip select (SPI reference)
            mosi     : out    std_logic;                                -- Master Our Slave In (SPI Reference)
            busy     : out    std_logic;                                -- segnala se la transazione � in corso
        ...
        if reset = '0' then                     -- reset dei valori  
            ...
            case state is                       
                ...
                when setup =>
                    ...
                    if prev_busy = '1' and curr_busy = '0' then             -- busy appena tirato gi�
                    ...
                    case counter is                                   
                        when 0 =>                                           -- mai avuta la linea occupata dalla scorsa configurazione, prima transazione (scegliamo il registro da modificare) 
                            ...
                            else                                            -- la prima transazione � iniziata
                                transmit    <= "0000" & parameter_data;     -- passiamo subito il valore da caricare nel registro (il buffer interno SPI ha gi� memorizzato) 
                            end if;
                        when 1 =>                                           -- linea si � liberata, tagliamo la comunicazione
                            spi_cont <= '0';                               
                            spi_ena  <= '0';                                
                            counter  := 0;                                  
                ...
                when read =>                                            -- lettura dei dati dell'accelerometro
                    ...
                    case counter is                                     
                        ...
                        when 6 =>                                       -- sesta volta che si libera la linea
                            spi_cont                <= '0';             -- togliamo in anticipo il flag per la modalit� continua, per interrompere il burst
                ...
                when output =>
                    cordic_ena     <= '1';         -- segnala che le uscite si sono aggiornate e il CORDIC può calcolare l'atan2
